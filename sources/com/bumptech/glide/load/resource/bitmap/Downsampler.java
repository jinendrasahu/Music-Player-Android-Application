...
public final class Downsampler {
    ...
    private Bitmap decodeFromWrappedStreams(ImageReader imageReader, BitmapFactory.Options options, DownsampleStrategy downsampleStrategy, DecodeFormat decodeFormat, PreferredColorSpace preferredColorSpace, boolean isHardwareConfigAllowed, int requestedWidth, int requestedHeight, boolean fixBitmapToRequestedDimensions, DecodeCallbacks callbacks) throws IOException {
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            options3.inPreferredColorSpace = ColorSpace.get(preferredColorSpace == PreferredColorSpace.DISPLAY_P3 && options3.outColorSpace != null && options3.outColorSpace.isWideGamut() ? ColorSpace.Named.DISPLAY_P3 : ColorSpace.Named.SRGB);
        } else {
    ...
    private static void calculateScaling(ImageHeaderParser.ImageType imageType, ImageReader imageReader, DecodeCallbacks decodeCallbacks, BitmapPool bitmapPool2, DownsampleStrategy downsampleStrategy, int degreesToRotate, int sourceWidth, int sourceHeight, int targetWidth, int targetHeight, BitmapFactory.Options options) throws IOException {
        ...
        int i = sourceWidth;
        int powerOfTwoHeight2 = sourceHeight;
        int i2 = targetWidth;
        int i3 = targetHeight;
        BitmapFactory.Options options2 = options;
        if (i <= 0) {
            int i4 = powerOfTwoHeight2;
        } else if (powerOfTwoHeight2 <= 0) {
            int i5 = powerOfTwoHeight2;
        } else {
            ...
            if (exactScaleFactor > 0.0f) {
                ...
                if (rounding != null) {
                    ...
                    if (imageType2 == ImageHeaderParser.ImageType.JPEG) {
                        ...
                        int powerOfTwoWidth2 = (int) Math.ceil((double) (((float) orientedSourceWidth) / ((float) nativeScaling)));
                        ...
                        if (secondaryScaling > 0) {
                            powerOfTwoWidth2 /= secondaryScaling;
                        ...
                        if (imageType2 == ImageHeaderParser.ImageType.PNG) {
                            ...
                            if (Build.VERSION.SDK_INT >= 24) {
                                ...
                                powerOfTwoWidth = (int) Math.floor((double) (((float) orientedSourceWidth) / ((float) powerOfTwoSampleSize)));
                ...
                int i15 = powerOfTwoHeight2;
                throw new IllegalArgumentException("Cannot round with null rounding");
            }
            int i16 = orientedSourceWidth;
            throw new IllegalArgumentException("Cannot scale with factor: " + exactScaleFactor + " from: " + downsampleStrategy2 + ", source: [" + i + "x" + powerOfTwoHeight2 + "], target: [" + i2 + "x" + i3 + "]");
        }
        if (Log.isLoggable(TAG, 3)) {
            Log.d(TAG, "Unable to determine dimensions for: " + imageType2 + " with target [" + i2 + "x" + i3 + "]");
        }
    }
